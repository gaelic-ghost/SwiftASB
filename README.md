...
SwiftASB is a pre-1.0 package with a supported interactive lifecycle and an active v1 API freeze in progress. `v0.9.1` is the current released baseline.
...
```swift
.package(url: "https://github.com/gaelic-ghost/SwiftASB", from: "0.9.1")
```
...
`ROADMAP.md` tracks milestone status and the next release-facing work. `v0.9.1` is already tagged as the current released baseline, and the roadmap plus git history remain the source of truth for what has shipped versus what is intentionally open.

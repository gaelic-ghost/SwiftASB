...
SwiftASB is a pre-1.0 package with a supported interactive lifecycle and an active v1 API freeze in progress. `v0.9.2` is the current released baseline.
...
```swift
.package(url: "https://github.com/gaelic-ghost/SwiftASB", from: "0.9.2")
```
...
The reviewed generated-wire baseline currently targets the `v0.125.0` Codex CLI schema. Newer Codex CLI schemas, including `v0.128.0`, are dumped and classified before their generated shapes are promoted or exposed through public Swift API.
...
`ROADMAP.md` tracks milestone status and the next release-facing work. `v0.9.2` is already tagged as the current released baseline, and the roadmap plus git history remain the source of truth for what has shipped versus what is intentionally open.

...
| Typed protocol request encoding | `Shipped internally` | `initialize`, `initialized`, `thread/start`, `thread/list`, `thread/read`, `thread/resume`, `thread/fork`, `thread/compact/start`, `thread/name/set`, `thread/metadata/update`, `thread/turns/list`, `model/list`, `mcpServerStatus/list`, and `turn/start` are encoded through the protocol layer. |
| Typed protocol response decoding | `Shipped internally` | `initialize`, `thread/start`, `thread/list`, `thread/read`, `thread/resume`, `thread/fork`, `thread/compact/start`, `thread/name/set`, `thread/metadata/update`, `thread/turns/list`, `model/list`, `mcpServerStatus/list`, and `turn/start` responses are decoded and validated against request IDs. |
...
| Thread name and metadata management | `Partially shipped` | `CodexThread.setName(...)` wraps `thread/name/set`, `CodexThread.updateMetadata(...)` wraps `thread/metadata/update`, and metadata patches use an explicit replace/clear/unchanged field model so callers can express upstream null-vs-omitted semantics. `thread/rollback` remains generated/internal until rollback history reconciliation is decided. |
...
| Public API curation | `Partially shipped` | The first source-organization pass has split app-wide model, MCP, and thread-management value types into dedicated `CodexAppServer+...` files while preserving `CodexAppServer` as the single connection-wide owner. More source splitting, name review, default-argument review, and symbol documentation remain before v1. |
...
- set thread names and patch stored Git metadata through `CodexThread`
...
8. Continue public API curation before v1: the first model/MCP/thread-management source split is done, but the package still needs more source splitting where it reduces file sprawl, tighter names and defaults, symbol documentation for the supported lifecycle and recent-history companions, and a final pass to make the first-class package surface feel intentionally designed rather than merely accumulated.

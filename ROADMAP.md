...
| Binary discovery and compatibility policy | `Partially shipped` | Explicit binary override exists, and the docs now define a rolling support window of the latest public Codex CLI release plus the prior two minor versions, but richer discovery diagnostics are still open. |

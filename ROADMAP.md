...
| Non-UI local history-reading helpers | `Partially shipped` | `CodexThread` now exposes direct `ClosedTurn` reads for one turn, a recent local window, and local older or newer windows around a known boundary turn id. This gives non-UI callers an intentional path into the local history store without binding a UI-oriented observable, while still deferring a broader public cursor model, transcript search surface, and richer history-query helpers. |
...
5. Expand the non-UI history surface only once the contract is ready: a deliberate public cursor model, transcript search, and richer local-history query helpers are still open.

...
| Promoted generated v2 wire snapshot | `Shipped internally` | `Sources/SwiftASB/Generated/CodexWire/Latest/` now contains a wider lifecycle batch covering bootstrap, stored and loaded thread reads, read-only filesystem requests, and many thread, turn, item, reasoning, and tool-progress notifications, alongside the hand-owned `CodexWireInitializeResponse` shim. |
...
| App-server filesystem reads | `Partially shipped` | `CodexAppServer.fs` now exposes the read-only `CodexFS` namespace for app-server-routed metadata, directory listing, and file-byte reads. This gives sandboxed clients a Codex-owned path for basic filesystem facts instead of requiring direct local disk reads. File mutations, watches, fuzzy file search, and repository-root discovery remain separate schema families for later promotion decisions. |

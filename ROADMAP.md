...
| App-wide observable companion | `In Progress` | `CodexAppServer.makeLibrary()` and `CodexAppServer.Library` now expose Core Data-backed value snapshots for unarchived, archived, and cwd-grouped threads, plus bindable sort/grouping policies, thread-list query descriptors, scoped refresh actions, and local reloads after app-wide thread/turn events. Remaining work includes repository-root grouping, selected-thread ordering, and app-wide model/MCP/hook capability snapshots. |

...
///
/// `CodexMCP` exposes opinionated MCP server installation without exposing the
/// app-server's raw config editing API.
public struct CodexMCP: Sendable {
...
public extension CodexAppServer {
    /// App-server-owned MCP configuration surface.
    var mcp: CodexMCP {
        CodexMCP(appServer: self)
    }
}

extension CodexMCP.ServerDefinition {
...
extension CodexMCP.StdioServer {
    var configValue: CodexAppServer.JSONValue {
...
extension CodexMCP.InstallOptions {
...
extension CodexMCP.InstallResult.WriteStatus {

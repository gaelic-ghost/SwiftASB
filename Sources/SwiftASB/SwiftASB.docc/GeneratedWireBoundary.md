...
- MCP server status snapshots through ``CodexExtensions/MCP/statusSnapshot()``
- MCP resource reads through ``CodexExtensions/MCP/readResource(server:uri:threadID:)``
- hook diagnostics snapshots through ``CodexAppServer/listHooks(_:)``
- app-wide observable inventory through ``CodexExtensions/makeInventory(configuration:)``

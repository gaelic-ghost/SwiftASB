...
struct CodexAppServerTests {
    ...
    func surfacesCodexCLIDiagnosticsAfterStart() async throws {
        let transport = FakeCodexAppServerTransport(
            executableResolution: .init(
                ...
                versionString: "codex-cli 0.132.0",
                compatibility: .supported(documentedWindow: "0.132.x")
            )
        )
        let client = CodexAppServer(transport: transport)
        ...
        #expect(diagnostics.versionString == "codex-cli 0.132.0")
    ...
    func startsAndInitializesThroughStartupRequest() async throws {
        ...
        #expect(startup.cliExecutableDiagnostics.versionString == "codex-cli 0.132.0")
    ...
    func startupRequestThrowsTypedUnsupportedCLIError() async throws {
        let transport = FakeCodexAppServerTransport(
            executableResolution: .init(
                ...
                compatibility: .outsideDocumentedWindow(documentedWindow: "0.132.x")
            )
        )
        let client = CodexAppServer(transport: transport)

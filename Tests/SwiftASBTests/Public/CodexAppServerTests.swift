...
struct CodexAppServerTests {
    ...
    func surfacesCodexCLIDiagnosticsAfterStart() async throws {
        let transport = FakeCodexAppServerTransport(
            executableResolution: .init(
                ...
                versionString: "codex-cli 0.143.0",
                compatibility: .supported(documentedWindow: "0.143.x plus 0.142.x when feasible")
            )
        )
        let client = CodexAppServer(transport: transport)
        ...
        #expect(diagnostics.versionString == "codex-cli 0.143.0")
    ...
    func startsAndInitializesThroughStartupRequest() async throws {
        ...
        #expect(startup.cliExecutableDiagnostics.versionString == "codex-cli 0.143.0")

...
struct CodexCLIExecutableResolverTests {
    ...
    func prefersExplicitExecutableURL() throws {
        ...
        #expect(resolution.versionString == "codex-cli 0.143.0")
        #expect(resolution.compatibility == .supported(documentedWindow: "0.143.x plus 0.142.x when feasible"))
    ...
    func marksSupportedVersionsInsideSupportWindow() throws {
        let explicitURL = URL(fileURLWithPath: "/tmp/codex-explicit")
        let recorder = CommandRecorder(pathVersionStandardOutput: "codex-cli 0.143.0")
    ...
    func marksLatestPriorMinorVersionsAsSupportedWhenFeasible() throws {
        let explicitURL = URL(fileURLWithPath: "/tmp/codex-explicit")
        let recorder = CommandRecorder(pathVersionStandardOutput: "codex-cli 0.142.9")
    ...
    func marksUnparseableVersionStringsAsUnknownFormat() throws {
        ...
        let resolution = try resolver.resolve()
        #expect(resolution.compatibility == .unknownVersionFormat(documentedWindow: "0.143.x plus 0.142.x when feasible"))
    }
}
...
private final class CommandRecorder: @unchecked Sendable {
    ...
    init(
        ...
        pathVersionStandardOutput: String = "codex-cli 0.143.0",

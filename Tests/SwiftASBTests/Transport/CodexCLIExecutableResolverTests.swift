...
struct CodexCLIExecutableResolverTests {
    ...
    func prefersExplicitExecutableURL() throws {
        ...
        #expect(resolution.versionString == "codex-cli 0.129.0")
        #expect(resolution.compatibility == .supported(documentedWindow: "0.129.x"))
    ...
    func marksSupportedVersionsInsideSupportWindow() throws {
        let explicitURL = URL(fileURLWithPath: "/tmp/codex-explicit")
        let recorder = CommandRecorder(pathVersionStandardOutput: "codex-cli 0.129.3")
    ...
    func marksOlderMinorVersionsOutsideSupportWindow() throws {
        ...
        let resolution = try resolver.resolve()
        #expect(resolution.compatibility == .outsideDocumentedWindow(documentedWindow: "0.129.x"))
    }
    ...
    func marksUnparseableVersionStringsAsUnknownFormat() throws {
        ...
        let resolution = try resolver.resolve()
        #expect(resolution.compatibility == .unknownVersionFormat(documentedWindow: "0.129.x"))
    }
}
...
private final class CommandRecorder: @unchecked Sendable {
    ...
    init(
        ...
        pathVersionStandardOutput: String = "codex-cli 0.129.0",

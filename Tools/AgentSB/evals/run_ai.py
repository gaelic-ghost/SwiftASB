...
async def _run_cases(cases: list[dict[str, Any]]) -> list[AIEvalResult]:
    coordinator = build_coordinator()
    results: list[AIEvalResult] = []
    for case in cases:
        result = await Runner.run(coordinator, case["prompt"])
        output = str(result.final_output).strip()
        results.append(_grade_case(case, output))
    return results


def _grade_case(case: dict[str, Any], output: str) -> AIEvalResult:
    ...
    return AIEvalResult(case["id"], not details, output, details)

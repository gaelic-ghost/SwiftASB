...
| App-wide MCP-server status snapshots | `CodexAppServer.mcpServerStatusSnapshot()`, `CodexAppServer.Library.mcpServers` | SwiftASB owns refresh for `mcpServerStatus/list`. `mcpServerStatusSnapshot()` keeps the full connection-wide catalog for inspectors; observable Library state exposes lightweight `McpServerSummary` values for common UI surfaces. The lower-level list request remains compatibility-only while consumers move to owned snapshots. |

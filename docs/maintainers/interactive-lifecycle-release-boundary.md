...
| App-wide MCP-server status snapshots | `CodexAppServer.mcpServerStatusSnapshot()`, `CodexAppServer.Library.mcpServers` | SwiftASB owns refresh for `mcpServerStatus/list` and exposes the latest connection-wide snapshot through app-server and observable library state. The lower-level list request remains compatibility-only while consumers move to owned snapshots. |

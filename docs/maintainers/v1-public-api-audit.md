...
supported app-server lifecycle before `v1.7.1`, not to expose every generated
wire family.
...
- [x] Add symbol comments for every stable v1 public type and method that is not
  ...
  Decision: complete for the `v1.7.1` release boundary. Default-bearing public
...
- [x] Update stale README release references before the next release.
  Decision: README now names `v1.7.1` as the current released baseline.
